...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'nooruluhda@gmail.com' ")
...
def get_latest_csv(folder_path):
    """Returns the latest CSV file from the given folder."""
    csv_files = glob.glob(os.path.join(folder_path, "*.csv"))
    if not csv_files:
        print("No CSV files found in the folder.")
        return None
    latest_file = max(csv_files, key=os.path.getmtime)  # Get the most recently modified file
    return latest_file

def send_file_via_tcp(file_path, server_ip, port):
    """Sends a file to the server via TCP."""
    if file_path is None:
        print("No file found to send.")
        return
...
# Set the correct folder path (Use raw string to avoid escape sequence issues)
attendance_folder = r"D:\5th Semester\spprroject\Face_recognition_based_attendance_system\Attendance"

# Get the latest CSV file
latest_csv = get_latest_csv(attendance_folder)

# Send the latest file to the server (Replace with the actual IP and port of the server)
if latest_csv:
    send_file_via_tcp(latest_csv, "192.168.43.126", 8080)
...
sendAttendanceButton = tk.Button(frame1, text="Send & Save Attendance", 
                                 command=lambda: send_file_via_tcp(get_latest_csv(attendance_folder), "192.168.43.126", 8080),
                                 fg="white", bg="green", width=35, height=1, 
                                 activebackground="white", font=('times', 15, ' bold '))
sendAttendanceButton.place(x=30, y=450)
